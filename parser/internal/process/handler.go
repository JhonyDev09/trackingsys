...
func NewHandler(st *store.Store) func(ctx context.Context, msg messages.RawMessage) error {
	return func(ctx context.Context, msg messages.RawMessage) error {
		switch msg.Protocol {
		case coban.Name:
			return hanleCoban(ctx, st, msg)
		default:
			log.Printf("[%s] protocolo desconocido %q, se descarte", msg.IMEI, msg.Protocol)
			return nil // no reintentar: nunca vamos a saber decodificarlo
		}
	}
}

func hanleCoban(ctx context.Context, st *store.Store, msg messages.RawMessage) error {
	switch msg.MsgType {
	case coban.MsgPosition:
		return handlePosition(ctx, st, msg)

	case coban.MsgLogin, coban.MsgHeartbeat:
		// nada que persistir todavía; sirven para mantener viva la
		// conexión en el receiver, no traen datos de negocio.
		return nil

	case coban.MsgAlarmSOS, coban.MsgAlarmPowerCut, coban.MsgAlarmPowerBack:
